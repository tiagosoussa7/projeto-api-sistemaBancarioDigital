...
const cadastro = async (req,res) => {
    ...
    try {
        ...
        await cadastrar_cliente(nome, cpf, email, data_nascimento, await criptar_senha(senha), await checar_banco());
...
const informacao_conta = async ( req, res ) => {
    const { cliente } = req;
    const conta = await checar_conta(cliente.id_cliente);
...
const extrato = async (req, res) => {
    ...
    const tabela_deposito = 'dados_depositos', tabela_saque = 'dados_saques';
    
    try {
        if ( transacao == 'deposito') return await detalhar_depositos(tabela_deposito, cliente, res);
        
        if ( transacao == 'saque') return await detalhar_saques(tabela_saque, cliente, res);
...
const atualizar = async (req, res) => {
    ...
    try {
        ...
        if (email !== cliente.email){
            if(await checar_email(email)) return res.status(400).json({mensagem: `Atualização negada: o email ${email} já está cadastrado.`});
        } 
        ...
        await knex('dados_cliente').where({id_cliente: cliente.id_cliente}).update({nome, email, data_nascimento, senha: await criptar_senha(senha)});
