...
const cadastro = async (req,res) => {
    ...
    try {
        ...
        if (!banco_cadastrado) {
            res.status(400).json({mensagem: 'Não há banco cadastrado no sistema.'});
        }
        ...
        if (idade_resposta(data_nascimento) < 18) {
            return res.status(400).json({mensagem: `${nome_resposta(nome)} você não tem idade suficiente para abrir uma conta no banco: ${nome_resposta(banco_cadastrado.nome)}.`});
        }
        ...
        const [ cliente_cadastrado ] = await knex('dados_cliente').insert({
           nome,
           cpf,
           email,
           data_nascimento,
        ...
        await knex('dados_conta').insert({
            numero_conta: cliente_cadastrado.id_cliente,
            id_banco: banco_cadastrado.id_banco
        });
        
        await knex('dados_banco').increment('qtd_contas', 1);
